...
new_build_lists = []
with open(os.path.join(src_path, 'build_csv_with_download_0530.csv'), 'w', encoding='utf-8-sig', newline = '') as csv_f:
            ...
            if element.get('resources'):
                ...
                if len(resources) > 0:
                    for key0, value0 in resources.items():
                        if key0 in ['resource_0', 'Download Video', 'Download Slides', 'Download Transcript']:
                            new_build_list[title_line.index(key0)] = value0
                        else:
                            value_index = title_line.index('resource_1')
                            value = new_build_list[value_index]
                            if value:
                                new_build_list[value_index] = f'{value}\n\n' + f'{key0}: {value0}'
                            else:
                                new_build_list[value_index] = f'{key0}: {value0}'
