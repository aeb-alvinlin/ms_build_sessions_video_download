...
new_build_lists = []
with open(os.path.join(src_path, 'new_build_csv.csv'), 'w', encoding='utf-8-sig', newline = '') as csv_f:
    ...
    for i, build_list in enumerate(build_lists):
        ...
        new_build_list[title_line.index('時長')] = duration_item
            ...
            if len(next_steps) > 0:
                for j, step in enumerate(next_steps):
                    for key, value in step.items():
                        if key == 'More on this topic':
                            new_build_list[title_line.index(f'下個主題{j+1}')] = value
                        else:
                            new_build_list[title_line.index(f'下個主題{j+1}')] = f'{key},{value}'
        ...
        if element.get('related_sessions'):
            ...
            if len(related_sessions) > 0:
                for k, step in enumerate(related_sessions):
                    for key, value in step.items():
                        new_build_list[title_line.index(f'相關{k+1}')] = f'{key},{value}'
            ...
            if len(tags) > 0:
                for key0, value0 in tags.items():
                    for key1, value1 in value0.items():
                        new_build_list[title_line.index(key0)] = f'{key1},{value1}'
