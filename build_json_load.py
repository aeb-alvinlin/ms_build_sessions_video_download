...
for i, build_list in enumerate(build_lists):
    ...
    if element.startswith('{'):
        print(f'== 讀取 {i}.{title_file_name} 正常! ==')
        continue

    print(f'== 讀取 {i}.{title_file_name} 開始! ==')
    ...
    try:
        ...
        try:
            ...
            for next_button in next_step_buttons:
                ...
                button_label = next_button.parent.find_elements(By.ID, 'nextStepLabel')
                if (len(button_label)>0):
                    button_label = button_label[0].text
